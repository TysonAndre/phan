...
abstract class AbstractPhanFileTest extends BaseTest implements CodeBaseAwareTestInterface
{
     ...
     */
    public abstract function getTestFiles();
    /**
    ...
    {
        $files = array_filter(array_filter(scandir($sourceDir), function ($filename) {
            // Ignore directories and hidden files.
            return !in_array($filename, ['.', '..'], true) && substr($filename, 0, 1) !== '.';
        }));
        return array_combine($files, array_map(function ($filename) use($sourceDir, $expectedDir) {
            return [[$sourceDir . DIRECTORY_SEPARATOR . $filename], $expectedDir . DIRECTORY_SEPARATOR . $filename . self::EXPECTED_SUFFIX];
        }, $files));
    }
    /**
