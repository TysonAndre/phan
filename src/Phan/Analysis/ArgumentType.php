...
class ArgumentType
{
     ...
     */
    public static function analyze(FunctionInterface $method, Node $node, Context $context, CodeBase $code_base)
    {
        // Special common cases where we want slightly
        // better multi-signature error messages
        if ($method->isPHPInternal()) {
            if (self::analyzeInternalArgumentType($method, $node, $context, $code_base)) {
                return;
            }
        }
        // Emit an error if this method is marked as deprecated
        if ($method->isDeprecated()) {
            Issue::maybeEmit($code_base, $context, Issue::DeprecatedFunction, $context->getLineNumberStart(), (string) $method->getFQSEN(), $method->getFileRef()->getFile(), $method->getFileRef()->getLineNumberStart());
        }
        ...
        if ($method->isNSInternal($code_base) && !$method->isNSInternalAccessFromContext($code_base, $context)) {
            Issue::maybeEmit($code_base, $context, Issue::AccessMethodInternal, $context->getLineNumberStart(), (string) $method->getFQSEN(), $method->getFileRef()->getFile(), $method->getFileRef()->getLineNumberStart());
        }
        ...
        $is_varargs = array_reduce(iterator_to_array($method->alternateGenerator($code_base)), function ($carry, FunctionInterface $alternate_method) {
            if (!is_bool($carry)) {
            ...
            $ret5902c6f192c51 = $carry || array_reduce($alternate_method->getParameterList(), function ($carry, $parameter) {
                if (!is_bool($carry)) {
                    throw new \InvalidArgumentException("Argument \$carry passed to () must be of the type bool, " . (gettype($carry) == "object" ? get_class($carry) : gettype($carry)) . " given");
                }
                return $carry || $parameter->isVariadic();
            }, false);
            if (!is_bool($ret5902c6f192c51)) {
                throw new \InvalidArgumentException("Argument returned must be of the type bool, " . gettype($ret5902c6f192c51) . " given");
            }
            return $ret5902c6f192c51;
        }, false);
        // Figure out if any of the arguments are a call to unpack()
        $is_unpack = array_reduce($arglist->children, function ($carry, $node) {
            return $carry || $node instanceof Node && $node->kind == \ast\AST_UNPACK;
        }, false);
        // Make sure we have enough arguments
        if (!$is_unpack && $argcount < $method->getNumberOfRequiredParameters()) {
            ...
            if (!$alternate_found) {
                if ($method->isPHPInternal()) {
                    Issue::maybeEmit($code_base, $context, Issue::ParamTooFewInternal, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $argcount, (string) $method->getFQSEN(), $method->getNumberOfRequiredParameters());
                } else {
                    Issue::maybeEmit($code_base, $context, Issue::ParamTooFew, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $argcount, (string) $method->getFQSEN(), $method->getNumberOfRequiredParameters(), $method->getFileRef()->getFile(), $method->getFileRef()->getLineNumberStart());
                }
            }
        }
        // Make sure we don't have too many arguments
        if (!$is_varargs && $argcount > $method->getNumberOfParameters()) {
            $alternate_found = false;
            foreach ($method->alternateGenerator($code_base) as $alternate_method) {
                $alternate_found = $alternate_found || $argcount <= $alternate_method->getNumberOfParameters();
            }
            if (!$alternate_found) {
                $max = $method->getNumberOfParameters();
                if ($method->isPHPInternal()) {
                    Issue::maybeEmit($code_base, $context, Issue::ParamTooManyInternal, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $argcount, (string) $method->getFQSEN(), $max);
                } else {
                    Issue::maybeEmit($code_base, $context, Issue::ParamTooMany, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $argcount, (string) $method->getFQSEN(), $max, $method->getFileRef()->getFile(), $method->getFileRef()->getLineNumberStart());
                }
            }
        }
        ...
        foreach (call_user_func(function ($v1, $v2) {
            ...
            if ($parameter->isPassByReference()) {
                if (!$argument instanceof \ast\Node || $argument->kind != \ast\AST_VAR && $argument->kind != \ast\AST_DIM && $argument->kind != \ast\AST_PROP && $argument->kind != \ast\AST_STATIC_PROP) {
                    Issue::maybeEmit($code_base, $context, Issue::TypeNonVarPassByRef, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $i + 1, (string) $method->getFQSEN());
                } else {
                    ...
                    if (Type::isSelfTypeString($variable_name) && !$context->isInClassScope() && $argument->kind == \ast\AST_STATIC_PROP && $argument->kind == \ast\AST_PROP) {
                        Issue::maybeEmit($code_base, $context, Issue::ContextNotObject, call_user_func(function ($v1, $v2) {
                            return isset($v1) ? $v1 : $v2;
            ...
            if (!$alternate_found) {
                ...
                if (is_object($parameter_type) && $parameter_type->hasTemplateType()) {
                    ...
                    Issue::maybeEmit($code_base, $context, Issue::TypeMismatchArgumentInternal, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $i + 1, $parameter_name, $argument_type_expanded, (string) $method->getFQSEN(), (string) $parameter_type);
                } else {
                    Issue::maybeEmit($code_base, $context, Issue::TypeMismatchArgument, call_user_func(function ($v1, $v2) {
                        return isset($v1) ? $v1 : $v2;
                    }, @$node->lineno, @0), $i + 1, $parameter_name, $argument_type_expanded, (string) $method->getFQSEN(), (string) $parameter_type, $method->getFileRef()->getFile(), $method->getFileRef()->getLineNumberStart());
                }
            }
        }
    }
     ...
     */
    private static function analyzeNodeUnionTypeCast($node, Context $context, CodeBase $code_base, UnionType $cast_type, \Closure $issue_instance)
    {
        ...
        $ret5902c6f194664 = $can_cast;
        if (!is_bool($ret5902c6f194664)) {
            throw new \InvalidArgumentException("Argument returned must be of the type bool, " . gettype($ret5902c6f194664) . " given");
        }
        return $ret5902c6f194664;
    }
    /**
            ...
            case 'implode':
                ...
                if ($argcount == 1) {
                    ...
                    if ((string) $arg1_type == 'array') {
                        if (!$arg2_type->canCastToUnionType(StringType::instance(false)->asUnionType())) {
                        ...
                        if (!$arg2_type->canCastToUnionType(ArrayType::instance(false)->asUnionType())) {
