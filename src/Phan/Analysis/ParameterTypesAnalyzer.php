...
class ParameterTypesAnalyzer
{
    ...
    private static function analyzeOverrideSignature(CodeBase $code_base, Method $method)
    {
        ...
        if ($type_option->isDefined()) {
            $o_parameter_list = array_map(function (Parameter $parameter) use($type_option, $code_base) {
                if (!$parameter->getUnionType()->hasTemplateType()) {
                    $ret5902c6f2337d0 = $parameter;
                    if (!$ret5902c6f2337d0 instanceof Parameter) {
                        throw new \InvalidArgumentException("Argument returned must be of the type Parameter, " . (gettype($ret5902c6f2337d0) == "object" ? get_class($ret5902c6f2337d0) : gettype($ret5902c6f2337d0)) . " given");
                    }
                    return $ret5902c6f2337d0;
                }
                ...
                $ret5902c6f233d97 = $mapped_parameter;
                if (!$ret5902c6f233d97 instanceof Parameter) {
                    throw new \InvalidArgumentException("Argument returned must be of the type Parameter, " . (gettype($ret5902c6f233d97) == "object" ? get_class($ret5902c6f233d97) : gettype($ret5902c6f233d97)) . " given");
                }
                return $ret5902c6f233d97;
            }, $o_parameter_list);
        }
     ...
     * @return void
     */
    private static function analyzeOverrideRealSignature(CodeBase $code_base, Method $method, Method $o_method, Clazz $o_class)
    {
        if ($o_class->isTrait()) {
            return;
            // TODO: properly analyze abstract methods overriding/overridden by traits.
        }
        // Get the parameters for that method
                ...
                foreach ($method->getRealParameterList() as $i => $parameter) {
                    ...
                    if (!$parameter_union_type->isEmpty()) {
                        if (!$o_parameter_union_type->isEqualTo($parameter_union_type) && !($parameter_union_type->containsNullable() && $o_parameter_union_type->isEqualTo($parameter_union_type->nonNullableClone()))) {
                            ...
                            $is_exception_to_rule = $parameter_union_type->hasIterable() && $o_parameter_union_type->hasIterable() && ($parameter_union_type->hasType(IterableType::instance(true)) || $parameter_union_type->hasType(IterableType::instance(false)) && !$o_parameter_union_type->containsNullable());
        ...
        if (!$o_return_union_type->isEmpty()) {
            if (!($o_return_union_type->isEqualTo($return_union_type) || $o_return_union_type->containsNullable() && !$o_return_union_type->nonNullableClone()->isEqualTo($return_union_type))) {
                $signatures_match = false;
            }
        }
        if ($o_method->returnsRef() && !$method->returnsRef()) {
        ...
        if (!$signatures_match) {
            if ($o_method->isPHPInternal()) {
                Issue::maybeEmit($code_base, $method->getContext(), Issue::ParamSignatureRealMismatchInternal, $method->getFileRef()->getLineNumberStart(), $method->toRealSignatureString(), $o_method->toRealSignatureString());
            } else {
                Issue::maybeEmit($code_base, $method->getContext(), Issue::ParamSignatureRealMismatch, $method->getFileRef()->getLineNumberStart(), $method->toRealSignatureString(), $o_method->toRealSignatureString(), $o_method->getFileRef()->getFile(), $o_method->getFileRef()->getLineNumberStart());
            }
        }
    }
}