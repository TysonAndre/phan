...
// to scan all nodes.
$visit_node = function (\ast\Node $node, $file_path) {
    if (!is_string($file_path)) {
        throw new \InvalidArgumentException("Argument \$file_path passed to () must be of the type string, " . (gettype($file_path) == "object" ? get_class($file_path) : gettype($file_path)) . " given");
    }
